...
namespace Zuxi.OSC.Modules.FriendRequests
{
    internal class HClient
    {
        ...
        public HClient()
        {
            ...
            httpClientHandler.CookieContainer.Add(new Cookie("auth", JsonConfig.AuthCookie) { Domain = "api.vrchat.cloud", Path = "/" });
            httpClientHandler.CookieContainer.Add(new Cookie("twoFactorAuth", JsonConfig.twoFactorAuthCookie) { Domain = "api.vrchat.cloud", Path = "/"});
            // Make the HTTP request using the HttpClient



            //  AHClient.DefaultRequestHeaders.UserAgent.ParseAdd("ZuxiJapi/1.2.7 vrchat@mail.imzuxi.com");
            AHClient.DefaultRequestHeaders.UserAgent.ParseAdd("ZuxiJapi%2F2.0.0%20vrchat%40mail.imzuxi.com");
